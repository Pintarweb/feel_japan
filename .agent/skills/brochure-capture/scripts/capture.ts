...
async function main() {
    ...
    for (const slug of itineraries) {
        ...
        // Filename: remove leading slash, replace remaining slashes with dash
        const cleanSlug = relativeSlug.replace(/^\//, '');
        const filename = cleanSlug.replace(/\//g, '-') + '.pdf';
        ...
        if (shouldCapture) {
            ...
            try {
                ...
                // Calculate full height for continuous PDF
                ...
                // Let's use 800px width for high quality, and full height.
